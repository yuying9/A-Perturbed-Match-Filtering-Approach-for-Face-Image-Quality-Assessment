...
quality_model = "ElasticFaceModel_PMF"        # Serfiq, BRISQUE, SDD, rankIQ, PFE, magface, FaceQnet, DeepIQA, CRFIQAL
threads = 20
quality_score_path = os.path.join("../data/quality_score", f"{dataset_name}_{quality_model}.txt")
path = os.path.join("your_IJB_path", dataset_name)
raw_feature_path = os.path.join("../data/quality_embeddings", dataset_name + "_" + model + "_raw")
...
def create_quality_list():
    ...
    save_path = os.path.join("../data/quality_score", dataset_name)
    pair_list = open(os.path.join(save_path, f"{dataset_name}_pair_{quality_model}.txt"), "w")
